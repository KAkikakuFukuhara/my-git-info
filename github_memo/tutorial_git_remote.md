...
- [5. リポジトリの連携](#5-リポジトリの連携)
  ...
  - [5.4. READMEの追加](#54-readmeの追加)
  - [5.5. ライセンスの追加](#55-ライセンスの追加)
  - [5.6. fetch](#56-fetch)
- [リモートリポジトリのダウンロード(git clone)](#リモートリポジトリのダウンロードgit-clone)
- [作業用ブランチのリモートリポジトリへのアップロード](#作業用ブランチのリモートリポジトリへのアップロード)
- [リモートリポジトリ上のブランチを削除](#リモートリポジトリ上のブランチを削除)
...
$ mkdir sample_app
$ cd sample_app/
sample_app$ echo "aaaa" >> test.txt
sample_app$ git init
sample_app$ git add test.txt 
sample_app$ git commit -m "[Add] first commit"
...
### 5.2. リモートリポジトリの作成と登録とアップロード
 ...
 　リモートリポジトリを作成する。作成の方法に関しては以下の4番以降を参考にする。なお以下のところは変更すること。  
 1. リポジトリは Public ではなく Private にすること。  
    1. Public は全世界に公開設定なので何らかの目的が無い場合以外は Private にすること  
 2. URLをコピーする際は https ではなく ssh を使用すること  
    1. httpsにすると毎回ログインが必要になるため  
...
```
```bash
$ git checkout -b feature/add-README.md
$ git add README.md
$ git commit -m "[Add] README.md"
$ git checkout master
$ git merge feature/add-README.md
...
$ git branch -d feature/add-README.md
```
...
```bash
$ git checkout -b feature/test2.txt
...
```bash
$ git merge feature/test2.txt -m "Merge branch 'feature/test2.txt'"
...
```bash
$ git checkout -b feature/test3.txt
$ echo "cccc" >> test3.txt
$ git add test3.txt 
...
```bash
$ git push origin feature/test3.txt
...
```bash
$ git checkout feature/test3.txt 
...
```bash
$ git push -d origin feature/test3.txt 
To github.com:KAkikakuFukuhara/sample_app.git
...
Switched to branch 'master'
Your branch is up to date with 'origin/master'.
$ git branch -D feature/test3.txt 
